...
object ConcurrentComposition extends App {
  ...
  def fetchTrack(): Future[String] = Future { "My Track" }
  def findComments(track: String): Future[Seq[String]] = Future { List("Nice!", "Awesome!", "Terrible!") }
  def sanitizeComment(comment: String): Future[String] = Future { comment.toLowerCase }
